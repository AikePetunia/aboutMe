...
router.get('/recent', async (_req, res) => {
  ...
  if (!steamKey || !steamId) {
    return res.status(500).json(
      err(provider, "local", {
        code: "AUTH",
        status: 500,
        message: "Steam API key or Steam ID not set",
      })
    );
  }

  const url = `https://api.steampowered.com/IPlayerService/GetRecentlyPlayedGames/v0001/?key=${steamKey}&steamid=${steamId}&format=json`;
  const r = await fetchJson(provider, url);

  if (!r.ok) {
    const code = r.status === 429 ? "RATE_LIMIT" : "UPSTREAM";
    return res.status(502).json(
      err(provider, r.requestId, {
        code,
        status: r.status || 502,
        message: r.networkError || "Steam fetch failed",
  ...
  const raw = r.json as SteamRawResponse;
  const games = raw.response.games?.map((g) => ({
    appId: g.appid,
    name: g.name,
